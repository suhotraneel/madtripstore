document.addEventListener("DOMContentLoaded", function () {
  ...
  var EXIT_TRANSITION = 1000;

  /* ---------- STATE ---------- */

  var pageLoaded = false;
  var minDurationPassed = false;
  var closing = false;

  /* ---------- VIDEO SETUP ---------- */

  // These attributes are critical for Chrome autoplay
  video.muted = true;
  video.loop = true;
  ...
  function startVideo() {
    ...
    if (playPromise !== undefined) {
      playPromise
        ...
        .catch(function () {
          // Autoplay blocked — show static first frame and carry on
          video.style.opacity = "1";
        });
    } else {
      video.style.opacity = "1";
  ...
  if (video.readyState >= 3) {
    ...
    video.load(); // Force Chrome to start loading
  }

  /* ---------- BULLETPROOF PAGE LOAD DETECTION ---------- */
  ...
  // Method 1: readyState already complete
  if (document.readyState === "complete") {
    onPageLoaded();
  }

  // Method 2: standard load event
  window.addEventListener("load", onPageLoaded);

  // Method 3: hard fallback — if nothing fires in 10s, force close
  var forceCloseTimer = setTimeout(function () {
    onPageLoaded();
  }, 10000);
  ...
  function tryClose() {
    ...
    if (closing) return;
    if (!(pageLoaded && minDurationPassed)) return;

    closing = true;
    clearTimeout(forceCloseTimer);

    loader.style.transition = "opacity " + EXIT_TRANSITION + "ms ease";
